...
# # The player has decided to resume the last game
# def test_resuming_game(fake_user_data_folder_with_game_1: Path):
#     """Test for resuming a game from the main menu."""

#     display = DisplayIO(term=Terminal(), console=Console(record=True))
#     bus = bootstrap(
#         uow=JsonUnitOfWork(Path(fake_user_data_folder_with_game_1)),
#         notifications=ConsoleNotifications(display),
#         rng=Random(42),  # Using a fixed seed to make the game state predictable
#     )
...
#     with patch.object(display.term, "inkey", side_effect=side_effects):
#         run_cli(bus=bus, display=display, test_mode=True)
#         raw_output = display.console.export_text()
#     loops = split_console_output_loops(raw_output, LOOP_TERMINATION_DELIMITER)
...
# # The user has progressed through the game over several sessions
# # They now decide to finish the game
# def test_finishing_game(fake_user_data_folder_with_game_2: Path):
#     """Test for finishing a game."""
#     display = DisplayIO(term=Terminal(), console=Console(record=True))
#     bus = bootstrap(
#         uow=JsonUnitOfWork(Path(fake_user_data_folder_with_game_2)),
#         notifications=ConsoleNotifications(display),
#         rng=Random(42),  # Using a fixed seed to make the game state predictable
#     )
...
#     # The user begins in the main menu, navigates to and selects 'Resume Game'
#     side_effects.append(fake_key(KEY_DOWN, "KEY_DOWN"))
#     side_effects.append(fake_key(KEY_ENTER, "KEY_ENTER"))

#     # The user makes a move to finish the game
#     side_effects.append(fake_key(KEY_UP, "KEY_UP"))
#     # A notification should be displayed indicating the game is over. See assertions.

#     # The user presses a key to continue
#     side_effects.append(fake_key(KEY_ENTER, "KEY_ENTER"))
#     # The user should now be back in the main menu. See assertions.
...
#     assert expected_message in loops[3]
